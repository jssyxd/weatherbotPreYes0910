...
def test_reconcile_cli_json_and_exit_codes():
    originals = _patch()
    try:
        with tempfile.TemporaryDirectory() as tmp, mock.patch.dict("os.environ", VALID_ENV, clear=False):
        ...
        try:
            buf = io.StringIO()
            with contextlib.redirect_stdout(buf), mock.patch.dict("os.environ", VALID_ENV, clear=False):
...
def test_order_plan_sell_and_caps():
    ...
    assert caps["no_max_ask"] == Decimal("1.0") and caps["yes_max_ask"] == Decimal("0.75"), caps
    assert order_plan.cap_for("buy_no", caps) == Decimal("1.0")
    assert order_plan.cap_for("buy_yes", caps) == Decimal("0.75")
...
def test_smoke_flow_denies_before_submit():
    ...
    cases = [
        ...
        ("non_marketable:marketable_would_fill",
         dict(env=_flow_env(), snapshot=lambda env, **kw: _snapshot(), price_override="0.60")),
    ]
    with tempfile.TemporaryDirectory() as tmp:
        log = Path(tmp) / "live_events.jsonl"
